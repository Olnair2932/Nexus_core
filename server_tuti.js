...
const axios = require("axios");               const os = require("os");
...
const BRAIN_LOG = path.join(BASE_DIR, "nexus_brain.log");
    ...
    const prompt = `Você é o NEXUS . Analise: "${texto}"                               BIBLIOTECA: [${bib}]
    HISTÓRICO: ${ctx}                             REGRAS:                                       1. Se o pedido se JÁ ESTÁ na BIBLIOTECA, use acao="tocar_musica".                              2. Se NÃO ESTÁ, use acao="baixar_musica".
acao=”mostrar_status”
    3.params DEVE ser o nome da música solicitada agora, e se possível pesquisa o nome do cantor no YouTube e completar o parâmetro da busca em toda a internet.
    Responda em JSON puro: {"acao":"", "params":"", "resposta":"", “conversar”:””, "reflexao":""}`;

    try {
        const res = await axios.post(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-lite:generateContent?key=${process.env.GEMINI_API_KEY}`,                             { contents: [{ parts: [{ text: prompt }] }] }, { timeout: 15000 });
        let raw = res.data.candidates[0].content.parts[0].text.replace(/```json|```/g, "").trim();
        return { success: true, data: JSON.parse(raw) };                                        } catch (e) { return { success: false }; }
}                                             
app.post("/api/chat", async (req, res) => {       const texto = req.body.texto || "";
