...
function obterContexto() {
    try {
        ...
        return fs.readFileSync(BRAIN_LOG, "utf8")
            .trim()
            .split("\n")
            .slice(-10)
            .join("\n");
...
2. Se NÃO ESTÁ, use acao="baixar_musica" e procure a fonte oficial disponível.
3. params DEVE ser o nome exato da música solicitada agora.
4. Ignore nomes de músicas antigas do histórico.
5. Não mencione plataformas específicas.

Responda em JSON puro:
{"acao":"","params":"","resposta":"","reflexao":""}`;
...
app.post("/api/chat", async (req, res) => {
    ...
    exec(
        `bash public/${script} "${intent.params}"`,
        { env },
        (err, stdout) => {
            ...
            if (stdout.includes("OK|")) {
                ...
                resp.nexus =
                    `🎶 [IA: ${intent.reflexao || "OK"}] Sintonizado: ${arq}`;

            } else if (intent.acao === "baixar_musica") {

                resp.nexus =
                    "📥 Comando de busca oficial enviado para localização da música.";
            }
...
app.listen(PORT, "0.0.0.0", () =>
    console.log(`🚀 NEXUS MASTER ONLINE`)
);
